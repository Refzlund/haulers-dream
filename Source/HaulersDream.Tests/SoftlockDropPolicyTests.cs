...
namespace HaulersDream.Tests
{
    ...
    {
        // A capable hauler (nothing disabled, normal priority, not a stuck mech) keeps its cargo: it will
        // unload on its own, so never yank items out from under it.
        [Test]
        public void CapableHauler_NoDrop()
        {
            Assert.That(SoftlockDropPolicy.ShouldDrop(
                haulingDisabled: false, haulingPriorityZero: false,
        ...
        {
            Assert.That(SoftlockDropPolicy.ShouldDrop(
                haulingDisabled: true, haulingPriorityZero: false,
                isMech: false, mechState: MechState.None,
                taggedCount: 1, runningHdJob: false), Is.True);
        }

        // Hauling work-type priority set to 0 (player set it to "never") → cargo never serviced → drop.
        [Test]
        public void PriorityZeroHolder_Drops()
        {
            Assert.That(SoftlockDropPolicy.ShouldDrop(
                haulingDisabled: false, haulingPriorityZero: true,
                isMech: false, mechState: MechState.None,
                taggedCount: 5, runningHdJob: false), Is.True);
        }
        ...
        public void RunningHdJob_NoDrop_EvenWhenEligible()
        {
            ...
            Assert.That(SoftlockDropPolicy.ShouldDrop(
                haulingDisabled: true, haulingPriorityZero: true,
        ...
        {
            // Fully capable.
            Assert.That(SoftlockDropPolicy.IsHaulIncapable(false, false, false, MechState.None), Is.False);
            Assert.That(SoftlockDropPolicy.IsHaulIncapable(false, false, true, MechState.None), Is.False, "awake mech is capable");
            // Each incapable signal independently flips it true.
            Assert.That(SoftlockDropPolicy.IsHaulIncapable(true, false, false, MechState.None), Is.True);
            Assert.That(SoftlockDropPolicy.IsHaulIncapable(false, true, false, MechState.None), Is.True);
            Assert.That(SoftlockDropPolicy.IsHaulIncapable(false, false, true, MechState.Charging), Is.True);
            // A mech-state on a non-mech does not count.
            Assert.That(SoftlockDropPolicy.IsHaulIncapable(false, false, false, MechState.Dormant), Is.False);
        }
    }
}
