...
#pragma warning restore CS0649
        ...
        public override IEnumerable<Toil> MakeNewToils()
        {
            ...
            AddFinishAction(delegate (JobCondition condition)
            {
                ...
                // portal that still has work (dedup excludes THIS portal). Byte-inert when the setting is off
                // (ShouldChain short-circuits). The chained job targets a different ledger key, so we still RELEASE this
                // portal's claims below (retainClaimOnEnd stays false — retaining would leak the finished claim); the
                // chained job re-claims its own portal in its Notify_Starting next tick.
