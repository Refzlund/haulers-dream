...
namespace HaulersDream
{
    ...
    public static class PackAnimalLoad
    {
        ...
        internal static bool HasDepositableSurplus(Pawn pawn)
        {
            ...
            // GetHashSet's self-heal (it mutates the tag set + tag-age bookkeeping) would be an MP desync. It is NOT the
            ...
            using (var surplusScan = InventorySurplus.BeginScan(pawn))
                foreach (var t in comp.PeekHashSet())
                    if (t != null && !t.Destroyed && inner.Contains(t) && surplusScan.SurplusOf(t, true) > 0)
                        return true;
            return false;
        }
